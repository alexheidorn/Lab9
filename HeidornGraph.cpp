...
namespace std {
    ...
    void findComponents(Vertex start) { 
        ...
        while (!open.empty())
            Vertex v = open.top();
            open.pop();
            if (explored[v.get] == false) //if the node HAS been explored, just ignore it!!!
                Explored[v] = true
                v.
                for each edge e from v //only adds edges IF it hasn't been explored previously
                    v2 = to vertex in e
                    if (Explored[v2] false)
                        open.push(v2)
